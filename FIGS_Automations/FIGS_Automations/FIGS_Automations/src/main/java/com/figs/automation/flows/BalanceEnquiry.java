...
public class BalanceEnquiry {
    ...
    public void run() {
        try {
            ...
            throw new RuntimeException("❌ Balance Enquiry flow failed", e);
        }
    }

    public void Enquiry() throws InterruptedException, IOException {
        ...
        try (FileInputStream fis = new FileInputStream(excelFile);
            ...
            XSSFSheet sheet = workbook.getSheet("Balance Enquiry");
            if (sheet == null) {
                throw new IllegalArgumentException("❌ Sheet 'Balance Enquiry' not found in file: " + excelFile.getAbsolutePath());
            }
            ...
            for (int rowindx = 2; rowindx <= sheet.getLastRowNum(); rowindx++) {
                ...
                if (arl.size() > 10 && "Y".equalsIgnoreCase(arl.get(10))) {
                    if (("AEPS".equalsIgnoreCase(arl.get(6))) || ("Account".equalsIgnoreCase(arl.get(6)) || ("Card".equalsIgnoreCase(arl.get(6))))) {
                        new CommonFlow(driver).BalanceEnquiryFlow(arl.get(6));
                    }
