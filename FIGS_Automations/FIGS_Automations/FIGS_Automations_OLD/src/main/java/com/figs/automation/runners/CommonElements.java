...
public boolean ValidateProceedButtonMoneyTransfer() {
    ...
    public void EnterWitnessAadhaarNumber(String aadhar) {
        try {
            WebElement WitnessName = driver.findElement(By.xpath(
                    "//label[contains(., 'Witness Aadhaar Number') or contains(., 'Beneficiary Aadhaar Number')]/following::input[1]"));
            WitnessName.clear();
	...
	public void EnterAccountNo()
	{
		try
		{
			WebElement Account =  driver.findElement(By.xpath("//input[@id='outlined-basic' and @type='text' and @maxlength='11']"));
			Account.sendKeys(arl.get(4));
		}
		catch(Exception e)
		{
	...
	public void ClickCollection() throws InterruptedException
	{
	  ...
	  public boolean ClickProceed() {
			...
			// Check for screen-level proceed button
			WebElement proceedBtn = driver.findElement(By.xpath("//button[@disabled and contains(@class, 'proceedbutton')]//p[contains(text(), 'PROCEED')]"));
			 boolean isenabled = proceedBtn.isEnabled();
				if (isenabled) {
					proceedBtn.click();
					return true;		
