...
//					break;
    ...
    public void ClickonRDSTDR() {
		
		Actions action = new Actions(driver);
		WebElement ekyc=driver.findElement(By.xpath("//div[normalize-space(text())='RD/STDR']"));
		action.moveToElement(ekyc).click().build().perform();	
   }
    
      public void ClickonOpenRD() {
		
		Actions action = new Actions(driver);
		WebElement ekyc=driver.findElement(By.xpath("//input[@type='radio' and @value='Open RD']"));
		action.moveToElement(ekyc).click().build().perform();	
     }
...
//            Thread.sleep(200);
    ...
    public  void RadioButton(String index) {
        try {
...
//	}
	...
	public void SelectFollowDate(String Date) throws InterruptedException {
	    // 👉 Example input: "9/10/2025 01:10 AM"
	    String[] parts = Date.split(" ");
	    ...
	    while (true) {
	        ...
	        if (currentHeader.equalsIgnoreCase(monthYear)) {
	            ...
	            // 👇 Adjust button direction if needed (Next/Previous)
